...
# Disable Git credential prompts to avoid hanging
...
if ($Help -or ($Command -eq $null -and $File -eq $null)) {
    ...
    Write-Host "  Automatically handles duplicate repository names by adding suffixes (_2, _3, etc.)" -ForegroundColor White
    Write-Host ""
...
# Function to generate unique directory name for git clone
function Get-UniqueDirectoryName {
    ...
    while (Test-Path (Join-Path $TargetPath $uniqueName)) {
...
$currentIndex = 0
$failedItems = @()
$clonedRepos = @{}  # Track cloned repositories to avoid conflicts
...
foreach ($item in $items) {
    ...
    try {
        if ($isGitCommand) {
            ...
            $uniqueDirName = Get-UniqueDirectoryName -RepoUrl $item -TargetPath $currentPath.Path
            $originalRepoName = ($item -split '/')[-1] -replace '\.git$', ''
            
            if ($uniqueDirName -ne $originalRepoName) {
                Write-Host "  ⚠️  Directory '$originalRepoName' already exists, using '$uniqueDirName'" -ForegroundColor DarkYellow
            }
            
            # Build clone command with custom directory name
            $arguments = "clone `"$item`" `"$uniqueDirName`""
            Write-Host "  → Running: git clone ... (saving as: $uniqueDirName)" -ForegroundColor Gray
            
            $process = Start-Process -FilePath $actualCommand -ArgumentList $arguments -NoNewWindow -Wait -PassThru
            
            if ($process.ExitCode -eq 0) {
                Write-Host "  ✅ Cloned: $uniqueDirName" -ForegroundColor Green
                $successCount++
                $clonedRepos[$originalRepoName] = $true
            } else {
